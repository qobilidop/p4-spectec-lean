...
inputs the hint names to `Option` of the outputs, trying the rule groups
in order and, within a group, the paths in order after the shared match
(`invoke_defined_rel`, sequential mode); the `else` group last. The
`Prop` encoding is generated by `Props` (M1 leaves it to the next step;
...
def relDecl (ctx : Ctx) (recursive externs : Bool) (id : String) (nottyp : nottyp)
  ...
  let header := Format.text s!"def {Names.relName id}.run" ++ binders ctx.env [] ins externs ++
    " : Option " ++ (typTerm.prod (outs.map (typTerm ctx.env []))).arg ++ " :="
