...
Status: agreed design, 2026-09-24. No code yet.
...
## 2. Upstream facts the design relies on
...
spec/*.watsup → Parse → Elaborate → IL ─┐
...
The `.watsup` files are the P4 *language* specification. A `.p4` file is a
...
| Upstream parser and elaborator (OCaml) | trusted | defines what the spec means; shared with the official P4 spec toolchain |
| JSON dump of the IL | trusted | tiny and structural; round-trip tested against upstream's IL printer |
| `P4SpecTec.Semantics` (IL interpreter in Lean) | trusted | the spec of the compiler; ported from upstream's AL interpreter; cross-checked by rung 2 |
| `P4SpecTec.Prelude.Builtins` | trusted | ports of upstream builtins; unit-tested against the originals |
| `P4Spec.Targets.*` | trusted | ports of upstream target code; tested by the packet leg of rung 2 |
...
On the semantics port: the IL *syntax* is a mechanical constructor-for-
constructor copy of `ast.ml`. The *semantics* is a port of the AL
interpreter, not a copy. The core (expression evaluation, iterators, path
updates, rule matching, backtracking) ports rule by rule. What the port
drops is engineering, not meaning: mutable context, result caching,
instrumentation hooks, backtraces, and the unique-id value-equality scheme
(Lean uses structural equality). Each omission is an easy judgment call
but not a mechanical one; that is what the side-by-side review and the
rung 2 interpreter comparison are for.

### Test sources (all from upstream)
...
```
p4-spectec-lean/
├── AGENTS.md                     # project instructions (no CLAUDE.md)
├── README.md
...
│   └── patches/
│       └── 0001-json-export.patch   # adds `elab --json` and `run --dump-value`
│
...
│   ├── IL/
│   │   ├── Ast.lean              # deep embedding; mirrors upstream ast.ml constructor for constructor
│   │   ├── Value.lean            # IL values (what booted programs decode from)
│   │   └── Json.lean             # FromJson for both
│   ├── Semantics/                # TRUSTED: IL interpreter, ported from upstream's AL interpreter
│   │   ├── Env.lean              # variable and definition environments
│   │   ├── Exp.lean              # expression evaluation, iterators, path updates
│   │   ├── Prem.lean             # premise checking, let-binding, if-guards
│   │   └── Rule.lean             # rule matching, backtracking, function clauses
...
│   │   ├── Builtins.lean         # TRUSTED: ports of upstream builtin decs
│   │   └── Target.lean           # class shape for extern syn / rel / dec
│   ├── Codegen/                  # NOT trusted: validated per definition
│   │   ├── Names.lean            # identifier and mixfix mangling
...
│   │   └── Command.lean          # `spectec_import "file.json"` elaborator command
│   ├── Tactic/
...
│
├── NanoP4Spec/                   # pilot: `spectec_import "exports/nano-p4.il.json"` + Targets/
├── P4Spec/                       # full spec, split per upstream spec section for build parallelism
│   └── Targets/                  # TRUSTED: V1Model, PSA, eBPF instances of the generated Target class
│
...
│
├── test/
│   ├── lean/                     # prelude vs builtins, decode round-trips, small #eval checks
│   └── diff/                     # rung 2 harness
...
│
└── scripts/
    ├── build-upstream.sh         # apply patches, build p4spectec
...
- Trusted files are marked. Only `Semantics/`, `Prelude/Builtins.lean`,
  and `*/Targets/` need side-by-side review against upstream.
...
- **M1, Nano-P4, rungs 1 and 2.** JSON dump patch, deep embedding, codegen
  for types, functions and relations, prelude, differential tests against
  upstream on the Nano-P4 corpus.
- **M2, Nano-P4, rung 3.** IL semantics in Lean, per-definition validation
  theorems, exec-to-Prop soundness theorems, generic tactic.
...
- Differential harness language: Python (matches upstream's fuzzer
  tooling) or a Lean executable. Leaning Python.
