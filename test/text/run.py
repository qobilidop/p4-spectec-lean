...
FIXTURE = ROOT / "test/text/observed.json"
...
def observations(upstream):
    ...
    shutil.copyfile(ROOT / "test/text/probe.ml", source)
...
def main():
    ...
    if args.check:
        if FIXTURE.read_text(encoding="utf-8") != output:
            raise SystemExit("text oracle fixtures are stale; rerun test/text/run.py")
    else:
